...
| WebAssembly, WASI, and the Component Model | Standards-track or standards-community specifications; host pages are vendor implementation references. | The repository has a versioned host-independent DBF core and a CI smoke check that loads the generated `wasm-bindgen` Node.js wrapper and exercises its ABI, query, single-mutation, and atomic-batch paths. Worker/WASI runtime adapters and asynchronous storage remain future. | Keep the core ABI and host-boundary decisions in [WASM](wasm.md). Add a host-specific source only when that host receives an adapter and a smoke test. |
