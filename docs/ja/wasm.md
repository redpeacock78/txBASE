...
- ホスト固有のタイムアウトと再試行を持つ、非同期ストレージ接続型クエリストリームがある。
...
Worker互換Web Streamsクエリアダプターとスモークフィクスチャもあります。
デプロイ済みワーカーまたはWASIランタイム、WASI固有のクエリストリームライフサイクル方針、ネイティブの復旧経路をすでにサポートするとは主張しません。
