...
`AsyncObjectTable::query_stream`は、ランタイムから独立した非同期ストレージ接続型クエリ向けに`AsyncObjectQueryStream`を返します。

最初のポーリングは、`AsyncObjectStore`を通じてコミット済みのXBFスナップショットを1つ復旧して読み取り、既存のXBFからDBFへの出力契約で変換した後、所有型スナップショットストリームへ行の配送を委譲します。
...
ランタイムから独立した非同期ストレージ接続型アダプターは、`AsyncObjectTable::query_stream`から利用できます。
