...
| 分散レプリケーションと権威 | [分散化の進化](distributed-evolution.md) | 現在のプロセス内バージョン付きエントリと再生の境界。ネットワーク転送、クォーラム、コンセンサス、フォロワー読み取りは将来 |
...
| Raft論文とRaftプロジェクト資料 | 論文とプロジェクトの一次資料です。 | リポジトリ所有の`ReplicationEntry`/`ReplicationLog`スライスは、現在は固定termのプロセス内単一writerを選択しています。Raftはネットワーク権威の候補にとどまり、クォーラムと分散実行はありません。 | ローカルのエントリと再生契約は[分散化の進化](distributed-evolution.md)に置き、Raftは現在の実装依存ではなく将来候補として扱います。 |
