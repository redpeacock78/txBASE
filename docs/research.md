...
| Distributed replication and authority | [Distributed evolution](distributed-evolution.md) | Current process-local versioned entry/replay boundary with journaled `TXRP` sidecar persistence, validated current and retained snapshot export, suffix-preserving authority-side log compaction, atomic local snapshot installation, bounded historical follower reads, bounded HTTP entry/snapshot delivery, optional RFC 6750 Bearer authentication, and default authority capture of `/transaction` and named-table mutations; the follower role rejects direct catalog writes; future TLS, follower-watermark coordination, quorum, consensus, and distributed follower-read guarantees |
...
| Raft consensus | A primary research paper and its project documentation. | The repository-owned `ReplicationEntry`/`ReplicationLog`/`ReplicationSnapshot` slice currently selects a fixed-term process-local single writer, journals its `TXRP` position with catalog mutations, local snapshot installation, or suffix-preserving local compaction, bounds historical reads by the applied position, and exposes bounded HTTP delivery for already-constructed entries and snapshots; Raft is still only a candidate for authority and no quorum or distributed execution exists. | Keep the local entry, replay, snapshot, compaction, `TXRP`, historical-read, and HTTP-delivery contracts in [Distributed evolution](distributed-evolution.md); keep Raft as a future candidate, not a current implementation dependency. |
