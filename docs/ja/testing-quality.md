...
CIは`cargo-fuzz`を使い、任意のXBFバイト列とクエリJSONをカバレッジ誘導型でファジングします。
XBFコーパスは、チェックイン済みの有効なフィクスチャと壊れたhexコーパスをデコードして作ります。
クエリのシードは`fuzz/corpus/query_json/`に置きます。
各対象は固定シード、1,000回の入力実行、最大1 MiBの入力、入力ごとに10秒のタイムアウトを使います。
...
クエリテストでは、生成したスカラー比較、包含、論理条件、null/欠損ケースを`src/query/reference_tests.rs`の独立したテスト専用評価器と比較します。
この参照評価器は配列述語と`$expr`を扱いません。

独立したUbuntuのファジングジョブは、nightlyで`cargo-fuzz` 0.13.2をインストールし、同じ上限で両方のファジング対象を実行します。
ジョブ全体の15分のタイムアウトで、セットアップとコンパイルも制限します。

SQLiteの完全なリリースプロセスより意図的に小さくしています。

無期限のファジング、soak、微分、障害注入ジョブは、入力、タイムアウト、成果物、失敗再現の契約を定義してから追加します。
...
5. 参照評価器を本番コードのヘルパーから独立させたまま、配列述語と`$expr`のクエリ微分検査を拡張する。
