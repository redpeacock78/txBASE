...
| XBF-005 | A representable XBF table exports to an in-memory DBF table with deletion state preserved; direct export rejects unsupported types and constraints, while schema-aware export returns representable constraint metadata. | `src/xbf/export.rs`; `src/xbf/tests.rs` | `exports_a_representable_xbf_table_to_dbf`; `exports_xbf_constraints_as_schema_metadata`; `rejects_nonrepresentable_xbf_dbf_export_types` | Boundary |
