...
| QRY-003 | Physical cursors stop after the requested page and look-ahead record; sorted cursors enforce a matching keyset definition; bounded `$match`, `$count`, `$group`, group-output projection, and local inner/left/right/semi/anti/cross joins enforce explicit bounds. | `src/query/pagination.rs`; `src/query/aggregation.rs`; `src/query/aggregation_plan.rs`; `src/query/join.rs` | `cursor_tests.rs`, `aggregation_tests.rs`, `right_join_keeps_unmatched_right_record`, and other join tests | Boundary |
...
- a full cost-based planner, multiple or planned joins, and aggregation stages beyond bounded `$match`, `$count`, `$group`, and group-output `$project`;
