...
| HTTP-003 | Successful table reads expose a strong ETag; state-changing routes honor optional If-Match with strong comparison, existing-resource wildcard semantics, 412 failure, no partial mutation, and a new ETag after commit. | `src/server.rs`; `src/server/records.rs`; `src/server/etag.rs`; `src/server/etag_tests.rs`; `docs/http-semantics.md` | `mutation_etag_prevents_lost_update`; `if_match_requires_strong_tags_and_supports_existing_wildcard`; `transaction_etag_precondition_is_atomic` | Boundary |
| HTTP-004 | GET routes honor If-None-Match with weak comparison, existing-resource wildcard semantics, 304 without a body, and the current ETag. | `src/server.rs`; `src/server/etag.rs`; `src/server/etag_tests.rs`; `docs/http-semantics.md` | `if_none_match_returns_not_modified_for_current_representation` | Boundary |
