...
| SCH-001 | Optional schema metadata validates active records and mutation candidates, applies scalar defaults for omitted inserts, and supports bounded table-level checks without changing legacy DBF bytes. | `src/dbf/schema_metadata.rs`; `src/dbf/mutation.rs`; `src/dbf/schema_metadata_tests.rs` | `loads_schema_metadata_and_enforces_local_constraints`; `schema_defaults_fill_missing_insert_fields`; `schema_checks_reject_invalid_candidates`; stale-sidecar and copy tests | Boundary |
