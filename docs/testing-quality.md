...
`src/query/join_strategy.rs::chooses_nested_loop_for_small_join_inputs`, `chooses_hash_for_large_join_inputs`, and `chooses_index_nested_loop_for_large_indexed_inputs` cover the deterministic equality-join strategy threshold; `large_single_key_join_uses_a_fresh_foreign_index` and its compound cases cover direct indexed paths, `chained_single_key_join_uses_a_fresh_foreign_index`, `chained_right_single_key_join_uses_a_fresh_foreign_index`, `chained_compound_join_uses_a_fresh_foreign_index`, and `chained_right_compound_join_uses_a_fresh_foreign_index` cover indexed chained stages, and the join tests cover output order and chained-stage semantics.
