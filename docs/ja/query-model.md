...
`$expr` のオペランドは、後述する有界な数値 `$add`、`$subtract`、`$multiply`、`$divide` の形式だけをサポートします。
...
| 式 | `$expr` | 同じレコードのスカラーリテラル、フィールド参照、または有界な数値 `$add` / `$subtract` / `$multiply` / `$divide` 式を比較する |
...
`$add`、`$subtract`、`$multiply`、`$divide` は、数値リテラル、フィールド参照、またはネストした数値式を二つだけ受け付けます。
