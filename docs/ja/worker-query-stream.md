...
両メソッドは所有型の`WasmObjectQueryStream`を返し、ランタイム非依存の非同期オブジェクトテーブルAPIと同じクエリパーサー、検証、XBF行の直接対応付け、共通の行ストリームを使います。
XBFクエリにDBF出力可能性は要求しません。
XBF固有のJSON形式は[XBF](xbf.md)に記載します。
...
オブジェクトストアのクエリでは、最初の行を返す前にコミット済みXBFスナップショット全体を読み込み、各pullで要求された行をJSONへ対応付けます。
リモートスナップショットをページ単位で読み込むわけではありません。
...
その他のアダプターとホストは、独自のI/Oキャンセル方針を定義します。
