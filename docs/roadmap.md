...
- A bounded XBF v1 codec, DBF-to-XBF conversion helper, durable snapshot path, and generation-checked full-snapshot WAL recovery with explicit size limits and section checksums; XBF-to-DBF export remains future.

The baseline intentionally does not include a full cost-based index model, streaming, multiple or planned joins, cross-table transactions, multi-stage aggregation, composite or cross-table constraints, XBF-to-DBF export, object-storage commits, or distributed replication.
...
The v1 wire contract is drafted in [XBF v1 format draft](xbf.md). A bounded codec, DBF-to-XBF conversion helper, durable snapshot path, and generation-checked full-snapshot WAL recovery exist for that draft. XBF-to-DBF export is not a current supported feature.
