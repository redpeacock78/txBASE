...
| WebAssembly, WASI, and the Component Model | Standards-track or standards-community specifications; host pages are vendor implementation references. | The repository has a versioned host-independent DBF core, a generated `wasm-bindgen` Node.js wrapper, JavaScript Promise-backed asynchronous XBF object-table and runtime-neutral query-stream adapters, a WASI 0.3 CLI query-stream component, a generic Worker-compatible Fetch object-store adapter, and a Worker-compatible Web Streams query adapter. CI checks the WASM ABI and adapters, plus the WASI component build and CLI smoke behavior. Deployed Worker support and production WASI storage integration remain future. | Keep the core ABI and host boundaries in [WASM](wasm.md), [WASI query streaming](wasi-query-stream.md), [Worker query stream adapter](worker-query-stream.md), and [Worker Fetch object-store adapter](worker-object-store.md). Keep provider-specific guarantees in a provider adapter document. |
