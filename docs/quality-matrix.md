...
| SCH-001 | Optional schema metadata validates active records and mutation candidates, including bounded table-level checks, without changing legacy DBF bytes. | `src/dbf/schema_metadata.rs`; `src/dbf/schema_metadata_tests.rs` | `loads_schema_metadata_and_enforces_local_constraints`; `schema_checks_reject_invalid_candidates`; stale-sidecar and copy tests | Boundary |
...
- a full cost-based planner, planned joins, and aggregation stages beyond bounded `$match`, `$count`, `$distinct`, `$group`, and group-output `$project`;
