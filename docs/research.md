...
| External secondary-index lifecycle | [Indexes](indexes.md) | Current sidecar maintenance, scalar and compound equality, compound equality-prefix candidates, compound equality-prefix range candidates, histogram-estimated range ordering, bounded Unicode collated ordered keys, ordered-prefix traversal, mixed-direction compound-prefix sorting, equality-prefix candidate choice, uniform-statistics ordering for equality candidates, single-index versus intersection cost choice, deterministic row-equivalent explanation fields for record reads, filter evaluations, traversal, logical page reads, and sort work, plus future locale-aware CJK collation and more precise physical modeling |
...
| [Unicode Standard Annex #15](https://www.unicode.org/reports/tr15/) and [Rust `char` casing APIs](https://doc.rust-lang.org/std/primitive.char.html) | Unicode normative specification and official Rust API documentation. | NFKC sort keys use the Unicode compatibility-normalization contract, while lowercase keys use Rust's locale-independent character mapping. txBASE exposes only these two bounded sort-key modes and does not claim locale-aware dictionary order or full collation compatibility. | Keep the accepted sort-key modes and their index-planning boundary in [query model](query-model.md) and [indexes](indexes.md); keep the source classification here. |
