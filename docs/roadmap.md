...
It implements one or more equality conditions, uses a fresh single-field index for large direct or chained non-right single-key probes when available, and otherwise uses a bounded nested-loop or in-memory hash map.
