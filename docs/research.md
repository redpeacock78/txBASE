...
| WebAssembly, WASI, and the Component Model | Standards-track or standards-community specifications; host pages are vendor implementation references. | The repository has a versioned host-independent DBF core, a generated `wasm-bindgen` Node.js wrapper, and a JavaScript Promise-backed asynchronous XBF object-table adapter. CI checks the ABI, snapshot round trip, four mutation methods, atomic-batch rollback, object publication, recovery, retention, and host-error mapping. Worker/WASI runtime adapters and their host-specific policies remain future. | Keep the core ABI and host-boundary decisions in [WASM](wasm.md). Add a host-specific source when a worker or WASI adapter and smoke test are added. |
