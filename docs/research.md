...
| WASM and worker host boundary | [WASM](wasm.md), [WASI query streaming](wasi-query-stream.md), [Worker query stream adapter](worker-query-stream.md), and [Worker Fetch object-store adapter](worker-object-store.md) | Current host-independent core, Promise-backed XBF adapter, WASI 0.3 CLI query-stream component with read-only filesystem access to current or retained XBF snapshots, Worker-compatible Web Streams query adapter, and generic Fetch adapter; deployed Worker, writable/provider-backed WASI storage, non-blocking WASI I/O, and provider-specific adapters remain future |
...
| WebAssembly, WASI, and the Component Model | Standards-track or standards-community specifications; host pages are vendor implementation references. | The repository has a versioned host-independent DBF core, a generated `wasm-bindgen` Node.js wrapper, JavaScript Promise-backed asynchronous XBF object-table and runtime-neutral query-stream adapters, and a WASI 0.3 CLI query-stream component that reads current or retained XBF snapshots through a read-only filesystem adapter. It also has a generic Worker-compatible Fetch object-store adapter and a Worker-compatible Web Streams query adapter. CI checks the WASM ABI and adapters, plus the WASI component build and CLI smoke behavior. Deployed Worker support, writable/provider-backed WASI storage, and non-blocking WASI I/O remain future. | Keep the core ABI and host boundaries in [WASM](wasm.md), [WASI query streaming](wasi-query-stream.md), [Worker query stream adapter](worker-query-stream.md), and [Worker Fetch object-store adapter](worker-object-store.md). Keep provider-specific guarantees in a provider adapter document. |
