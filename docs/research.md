...
| Raft consensus | A primary research paper and its project documentation. | The repository-owned `ReplicationEntry`/`ReplicationLog`/`ReplicationSnapshot` slice currently selects a fixed-term process-local single writer, journals its `TXRP` position with catalog mutations or local snapshot installation, and bounds historical reads by the applied position; Raft is still only a candidate for network authority and no quorum or distributed execution exists. | Keep the local entry, replay, snapshot, `TXRP`, and historical-read contract in [Distributed evolution](distributed-evolution.md); keep Raft as a future candidate, not a current implementation dependency. |
