...
直接結合のモデルは、単一テーブルの`QUERY /explain`コストオブジェクトには含めません。

すべての中間ステージへ基数とマテリアライズの入力を渡せるようになるまでは、多段結合ステージは有界な行数ベースの戦略選択を保ちます。
...
3. 多段結合の基数とマテリアライズを考慮したコスト計画、ファイルシステムとキャッシュを考慮したmerge計画、より広い結合意味論。
