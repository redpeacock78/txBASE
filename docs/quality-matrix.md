...
| XBF-007 | Schema-preserving XBF-to-DBF file export uses a durable `TXSE` journal, exact target base bytes, per-file sync-and-replace, memo-sidecar cleanup, durable transaction-state replacement, existing-index refresh, DBF-read recovery after partial replacement, and conflict rejection for externally changed targets. | `src/dbf/schema_export.rs`; `src/dbf/persistence.rs`; `src/dbf/parser.rs`; `src/index/commit.rs`; `src/dbf/schema_export_tests.rs` | `schema_export_commits_dbf_and_schema_together`; `schema_export_removes_stale_memo_sidecars`; `schema_export_read_recovers_after_dbf_replacement`; `schema_export_read_rejects_an_external_target_change` | Boundary |
| IDX-001 | External scalar and compound indexes are rebuildable, freshness-checked, rejected before an unsafe mutation, and refreshed after supported mutations or recovery. | `src/index.rs`; `src/index/`; `src/index_tests.rs`; `src/dbf/tests/persistence.rs` | `mutation_refreshes_the_sidecar_after_save`; `rejects_an_invalid_index_before_saving_the_dbf`; `replays_operation_intent_when_state_payload_is_missing`; `direct_dbf_change_leaves_the_sidecar_stale_until_rebuild` | Current |
...
| IDX-004 | When equality, range, and ordered access paths coexist, the planner selects the smallest exact candidate set while preserving table-scan results. | `src/query/planner.rs`; `src/query/planner_compound_tests.rs`; `docs/indexes.md` | `chooses_the_access_path_with_fewer_exact_candidates` | Boundary |
...
| CAT-004 | The catalog server exposes the selected query plan for a named table through the same planner explanation contract as the single-table server. | `src/server/catalog.rs`; `src/server/explain.rs`; `docs/catalog.md`; `docs/http-semantics.md` | `catalog_server_reads_named_tables_through_record_routes` | Boundary |
| CAT-005 | Named-table mutation operations can commit across multiple DBFs through one catalog journal, roll back validation failures, and recover prepared or committed journal states. | `src/catalog/transaction.rs`; `src/server/catalog_transaction.rs`; `docs/catalog.md`; `docs/http-semantics.md` | `commits_named_operations_across_tables`; `rejects_a_failed_named_transaction_without_persisting_earlier_tables`; catalog journal recovery tests; `catalog_server_transaction_commits_multiple_named_tables`; `catalog_server_transaction_rolls_back_when_a_named_operation_fails` | Boundary |
