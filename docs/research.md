...
| Distributed replication and authority | [Distributed evolution](distributed-evolution.md) | Current process-local versioned entry/replay boundary with journaled `TXRP` sidecar persistence and bounded historical follower reads; future network transport, quorum, consensus, snapshot installation, and distributed follower-read guarantees |
...
| Raft consensus | A primary research paper and its project documentation. | The repository-owned `ReplicationEntry`/`ReplicationLog` slice currently selects a fixed-term process-local single writer, journals its `TXRP` position with the catalog mutation, and bounds historical reads by the applied position; Raft is still only a candidate for network authority and no quorum or distributed execution exists. | Keep the local entry, replay, `TXRP`, and historical-read contract in [Distributed evolution](distributed-evolution.md); keep Raft as a future candidate, not a current implementation dependency. |
