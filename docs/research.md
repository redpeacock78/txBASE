...
| Distributed replication and authority | [Distributed evolution](distributed-evolution.md) | Current process-local versioned entry/replay boundary; future network transport, quorum, consensus, and follower reads |
...
| Raft consensus | A primary research paper and its project documentation. | The repository-owned `ReplicationEntry`/`ReplicationLog` slice currently selects a fixed-term process-local single writer; Raft is still only a candidate for network authority and no quorum or distributed execution exists. | Keep the local entry and replay contract in [Distributed evolution](distributed-evolution.md); keep Raft as a future candidate, not a current implementation dependency. |
