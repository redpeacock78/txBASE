...
| Cloudflare R2 Workers APIと整合性モデル | プロバイダーの公式実装文書です。 | R2アダプターは、文書化されたバケットバインディング操作、条件付き`put`、引用符付き`httpEtag`、`truncated`と`cursor`によるページ分割を使います。CIではインメモリのバインディングフィクスチャで対応付けを検査しますが、デプロイ済みWorkerやR2サービスには接続しません。 | R2固有の整合性とバインディング動作は[Cloudflare R2オブジェクトストレージアダプター](r2-object-store.md)に置きます。汎用FetchやRustのオブジェクトストレージ契約へプロバイダー固有の保証を持ち込みません。 |
