...
`PACK`はmemoサイドカーを圧縮しません。

既存のインデックスサイドカーはpack済みDBFの保存後に更新しますが、memoブロックはそのままです。
...
このコマンドは読み取り専用で、WALを作成も切り詰めもしません。

したがって削除済みmemoブロックは、サイドカー固有の圧縮契約ができるまで再利用可能な孤立領域として残ることがあります。
