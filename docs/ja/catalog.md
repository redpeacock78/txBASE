...
過去のカタログイメージは現在、Rust APIとCLIから公開します。

カタログHTTPサーバーは、引き続き現在のイメージだけを提供します。
...
カタログはタプル全体を比較し、子側の値のいずれかがnullなら検査を省略し、親の更新や論理削除に対する同じ孤立防止規則を適用します。
...
完全なコストベースプランナー、完全なインデックス対応またはコストベースのマージ結合戦略、runtime固有のasync stream trait、行単位のMVCCバージョン、分散可視性は提供しません。
