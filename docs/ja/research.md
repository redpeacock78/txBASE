...
| 外部セカンダリインデックスのライフサイクル | [インデックス](indexes.md) | 現在のサイドカー保守、スカラーおよび複合キーの等値、複合等値プレフィックス候補、複合等値プレフィックス範囲候補、ヒストグラムによる範囲順序、有界なUnicode照合付き順序キー、バージョン付きICU4X 2.1.1の日本語・中国語・韓国語照合、順序プレフィックス、混在方向の複合プレフィックスソート、等値プレフィックス候補選択、一様統計による等値候補の順序付け、単独インデックスと積集合のコスト選択、レコード読み取り数、レコードページ読み取り、フィルター評価数、走査、インデックスページ読み取り、ソート作業の決定的な行相当の説明フィールド、より広い上流CJKフィクスチャとより精密な物理モデル |
