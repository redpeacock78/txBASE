...
グループ出力には 0 個以上の `$match` を置き、その後に任意の `$project` を一つ、最後の `$sort` を最大一つ、`$skip` を最大一つ、最後の `$limit` を最大一つ置けます。
...
`$skip` は 0 以上の整数を受け付け、ソート後かつ `$limit` の前に、具体化されたグループ結果を指定件数だけ破棄します。
...
`$limit` 後のステージ、追加の group、count、distinct、skip ステージは未サポートです。
