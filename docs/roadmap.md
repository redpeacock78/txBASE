...
- An optional `*.txschema.json` sidecar with one-field `primary`, `unique`, and `not_null` enforcement, bounded composite `unique` keys, scalar defaults for omitted inserts, and bounded table-level `checks` predicates.
