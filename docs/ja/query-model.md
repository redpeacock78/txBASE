...
候補ペアがそれより多い場合は `Hash` を選び、`right` 結合では左テーブル、それ以外では右テーブルについて一つのメモリ内等値マップを作ります。
...
これは有界なカーディナリティベースの戦略選択器であり、完全なコストベースプランナー、インデックスネステッドループプランナー、ストリーミング実行器ではありません。
