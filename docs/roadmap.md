...
- Bounded `$expr` boolean trees over field-to-field comparison leaves, including numeric `$add`, `$subtract`, and `$multiply` operands, that remain on the table-scan reference path.
...
- Full expression evaluation beyond the bounded `$expr` boolean-tree form and its numeric `$add`/`$subtract`/`$multiply` operands.
- Joins.
- Constraints.
