...
- ジャーナル化された`TXRP`サイドカー状態、検証済みカタログスナップショット、原子的なローカルスナップショットインストール、再起動時の位置検査、適用済み位置に限定した過去時点フォロワー読み取り、有界なHTTPエントリおよびスナップショット配送ルートを備えた、プロセス内固定termのレプリケーションログ。
...
`--replication-term TERM`で固定ローカルレプリケーションtermを選択でき、既定値は`1`です。
...
これらのルートは構築済みのエントリまたはスナップショットを配送しますが、通常のカタログ更新をレプリケーションログへ自動追記せず、クォーラム、コンセンサス、認証も提供しません。
...
`--after`は排他的なトランザクションIDカーソルであり、利用者の状態を確認応答として記録せず、保持もしません。
...
カタログジャーナル上のプロセス内固定termによるレプリケーションエントリと再生の境界、検証済みカタログスナップショットのローカルインストール、ジャーナル化された`TXRP`サイドカー、適用済み位置に限定した過去時点フォロワー読み取りも提供します。
ネットワーク転送、クォーラム、コンセンサス、ネットワーク経由のスナップショット転送、分散フォロワー読み取りの保証は現在のスライスに含みません。
...
- ネットワークレプリケーション、クォーラムまたはコンセンサス、ネットワーク経由のスナップショット転送、分散フォロワー読み取り、分散パーティショニング。
