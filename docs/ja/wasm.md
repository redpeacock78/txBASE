...
リポジトリには、ホストから独立したDBFコアのスライスがあります。
ワーカーと非同期ストレージのアダプターは今後の作業です。
...
- 条件付き公開を持つ非同期オブジェクトストレージアダプターがある。
...
リポジトリにはWASMコアの実装とターゲットコンパイル検査がありますが、ワーカーまたはWASIランタイム、非同期ストレージ、ネイティブの復旧経路をすでにサポートするとは主張しません。
