...
| Runtime-neutral asynchronous query streaming | [Asynchronous query streaming](async-streaming.md) and [Worker query stream adapter](worker-query-stream.md) | Current polling boundary, native threaded adapter, Worker-compatible Web Streams adapter, and runtime-neutral asynchronous-storage adapter for current or retained XBF generations; WASI-specific scheduling and host lifecycle remain future |
...
| WASM and worker host boundary | [WASM](wasm.md), [Worker query stream adapter](worker-query-stream.md), and [Worker Fetch object-store adapter](worker-object-store.md) | Current host-independent core, Promise-backed XBF adapter, Worker-compatible Web Streams query adapter, and generic Fetch adapter; deployed Worker, WASI, and provider-specific adapters remain future |
