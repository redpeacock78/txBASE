...
| DBF-003 | Declared CJK drivers and sidecar encoding overrides use byte-width validation, replacement reads, and rejecting writes. | `src/dbf/codec_cjk.rs`; `src/dbf/cjk_tests.rs`; `src/dbf/schema_metadata_tests.rs` | `decodes_and_encodes_declared_cjk_drivers`; `applies_a_supported_encoding_override_to_reads_and_writes` | Current |
