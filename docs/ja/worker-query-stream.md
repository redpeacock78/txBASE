...
ストリーム生成がオブジェクトストアのPromiseを待っている場合、アダプターは行の転送を止め、初期化後にクエリストリームをキャンセルしますが、実行中のストレージ操作までは中断できません。
`AsyncObjectStore`には操作単位のキャンセルトークンがないため、リモートI/Oも停止する必要があるホストは独自のリクエストキャンセル方針を適用します。
...
WASIスケジューラー、ネットワーククエリエンドポイント、リモートページ読み取り、カーソル再開、実行中のオブジェクトストア操作のキャンセルは提供しません。
