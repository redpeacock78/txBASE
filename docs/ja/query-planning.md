...
`explain_query_details_at`と`cost`オブジェクトは、アクセス経路の選択に使った行相当の作業量を公開します。
...
これらの統計は、決定的な行相当コスト推定に使います。

プランナーは、テーブルスキャンのアクティブレコード数とインデックス経路の正確な候補数を比較します。
選択したサイドカーのエントリ数から求めた有界な対数走査項を加え、候補レコードの読み取り数とフィルター評価数を数え、経路が要求された順序の一部しか提供しない場合はメモリ上のソート作業も加えます。
...
このモデルは決定的な候補作業を推定しますが、物理インデックスI/O、メモリ使用量、キャッシュ状態、照合、複合範囲の選択性は推定しません。
...
txBASEは現在、アクティブレコード数、等値用の一様な異なるキー数推定、単一フィールドの範囲ヒストグラム、複合等値プレフィックス候補と複合等値プレフィックス範囲候補、複合定義ごとの方向メタデータ、スキャン、インデックス走査、候補レコードの読み取り、フィルター評価、残りのソート作業に対する説明可能なコスト推定を持ちます。
...
2. 現在の有界な集約契約と有界な数値`$sum`および`$avg`式を超える、メモリ動作を有界にした追加の集約ステージとアキュムレータ。
