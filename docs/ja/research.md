...
| 単一テーブルのスナップショットトランザクションと分離 | [スナップショットトランザクション](transactions.md)と[MVCC](mvcc.md) | 現在のオプティミスティックな境界と任意選択の粗粒度serializableテーブルロック、カタログHTTPの過去読み取り、将来の述語単位またはテーブル横断のserializable作業 |
...
| PostgreSQLのトランザクション分離とMVCC文書 | 公式プロジェクト文書です。 | トランザクションとMVCCの文書で、既定のオプティミスティックな古い元データの検査と任意選択の粗粒度serializableテーブルロックを、未実装の述語単位およびテーブル横断のserializable保証から区別するために使います。 | 区別は[スナップショットトランザクション](transactions.md)と[MVCC](mvcc.md)に置き、PostgreSQLの分離保証を現在の契約へコピーしません。 |
