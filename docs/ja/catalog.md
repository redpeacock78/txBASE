...
トランザクション開始時にDBFの名前とファイル名の集合を取得し、commit時に集合が変わっていれば、テーブルを暗黙に除外または追加せずに拒否します。
...
完全なコストベースプランナー、完全なインデックス対応またはコストベースのマージ結合戦略、ホスト固有の`AsyncQueryStream`スケジューリング、行単位のMVCCバージョン、分散可視性は提供しません。
