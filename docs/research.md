...
| Snapshot transactions and isolation | [Snapshot transactions](transactions.md), [MVCC](mvcc.md), and [Catalog](catalog.md) | Current optimistic table boundary, opt-in coarse-grained serializable table and catalog locks, catalog HTTP historical reads, and future predicate-level or dynamic-table-set serializable work |
...
| PostgreSQL transaction isolation and MVCC documentation | Official project documentation. | The transaction, MVCC, and catalog documents distinguish the default optimistic stale-source check and the optional coarse-grained serializable table or catalog lock from predicate-level, dynamic-table-set, and distributed serializable guarantees that are not implemented. | Keep the distinction in [Snapshot transactions](transactions.md), [MVCC](mvcc.md), and [Catalog](catalog.md); do not copy PostgreSQL isolation guarantees into the current contract. |
