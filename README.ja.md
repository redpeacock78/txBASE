...
- ジャーナル化された`TXRP`サイドカー状態、現在と保持済みの検証済みカタログスナップショット、suffixを保つローカルログ圧縮、フォロワー適用位置の確認、原子的なローカルスナップショットインストール、再起動時の位置検査、適用済み位置に限定した過去時点フォロワー読み取り、有界なHTTPエントリ、スナップショット、フォロワー適用位置確認ルートを備えた、プロセス内固定termのレプリケーションログ。
