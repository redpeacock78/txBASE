...
| XBF-004 | ロード済み DBF レコードが、物理順、削除フラグ、DBF が区別できる null、表現可能なフィールド単位のスキーマ制約を落とさず有界 XBF 値へ変換され、未サポートの値とスキーマメタデータは明示的に失敗する。 | `src/xbf/conversion.rs`; `src/dbf/schema_metadata.rs`; `src/xbf/export_tests.rs` | `converts_a_dbf_fixture_to_xbf_without_dropping_records`; `converts_representable_dbf_schema_constraints_to_xbf`; `rejects_dbf_schema_constraints_without_an_xbf_v1_representation` | Boundary |
