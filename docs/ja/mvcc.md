...
`Catalog::begin_serializable`は、カタログに対する任意選択の粗粒度serializable境界です。
beginからcommit、rollback、またはdropまでカタログwrite lockと検出したすべてのテーブル排他ロックを保持し、非公開テーブルコピーへ操作を適用して1つのカタログjournalでcommitします。
...
述語単位のロック、動的なテーブル集合の検証、分散調整は提供しません。
...
スキーマ移行履歴、述語単位のロック、動的なテーブル集合の検証、長寿命の分散トランザクションは将来の作業です。

分散スナップショット、フォロワー読み取り、分散serializable調整は、さらに後の作業です。
