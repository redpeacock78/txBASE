...
- A rebuildable external scalar and compound-key index sidecar with equality and range candidate lookup, histogram-estimated range ordering, single-field and ordered-prefix traversal, compound-prefix sort traversal, uniform-statistics-ordered equality candidate intersection, path-aware planning, and DBF/memo freshness checks.

The baseline intentionally does not include full cost-based index choice, mixed-direction compound indexes, cursors, multi-record transactions, aggregation, joins, constraints, XBF, object-storage commits, or distributed replication.
...
The index sidecar foundation is implemented for scalar and ascending compound keys, exact equality and range candidate lookup, histogram-estimated range ordering, single-field ordered traversal, ordered-prefix traversal for multi-key sorts, compound-prefix traversal for all-ascending or all-descending sorts, uniform-statistics-ordered equality candidate intersection, path-aware planning, stale detection, explicit rebuild, and WAL-backed DBF/index recovery after normal persistence.

It does not yet support a full cost model, mixed-direction compound definitions, or cross-table atomic commits.

The remaining items need a public contract, malformed-input behavior, crash behavior, and a fixture or deterministic test.

An index is not complete for the broader roadmap until insert, update, logical delete, recovery, stale-index detection, rebuild behavior, full cost-based choice, mixed-direction definitions, and crash behavior are specified and tested together.
...
- Joins.
- Constraints.
- Range, histogram-based, and mixed-direction compound query planning.
...
- Full histogram and cost-based planners, joins, aggregation, MVCC, XBF, object-storage, or distributed code without a contract and end-to-end test.

The next index slice is intentionally local: compound-aware cost choice and mixed-direction ordering, after their contracts are written.
