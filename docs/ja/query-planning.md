...
これらはローカルな選択性推定であり、完全なコストモデルや MongoDB プランナー互換性ではありません。
...
等値、範囲、順序付きアクセス経路が同時に有効な場合、プランナーは正確な候補数を比較して最小の候補集合を選びます。

候補数が同じ場合、複合定義は最短の定義、安定した名前の順で選びます。

この候補数選択は完全なコストモデルではありません。

インデックス I/O、メモリ、キャッシュ状態、照合、複合キーの範囲選択性を推定しないためです。
...
txBASE は現在、アクティブレコード数、等値用の一様な異なるキー数推定、単一フィールドの範囲ヒストグラム、複合定義ごとの方向メタデータを持ちます。
...
4. 長寿命ストリーム向けのランタイム固有非同期トレイトと HTTP チャンクストリーミング。
5. 小さな参照評価器との微分テスト。
