...
| WebAssembly, WASI, and the Component Model | Standards-track or standards-community specifications; host pages are vendor implementation references. | The repository has a versioned host-independent DBF core, a generated `wasm-bindgen` Node.js wrapper, a JavaScript Promise-backed asynchronous XBF object-table adapter, and a generic Worker-compatible Fetch object-store adapter. CI checks the ABI, snapshot round trip, four mutation methods, atomic-batch rollback, object publication, recovery, retention, host-error mapping, HTTP conditional publication, timeout, and cancellation. A deployed Worker/WASI runtime adapter and host-specific query-stream policy remain future. | Keep the core ABI and generic transport boundary in [WASM](wasm.md) and [Worker Fetch object-store adapter](worker-object-store.md). Keep provider-specific guarantees in a provider adapter document. |
| Fetch, DOM, and Cloudflare Workers host documentation | Fetch and DOM are Web platform specifications; Cloudflare Workers documentation is an official host implementation reference. | The Worker adapter uses `fetch`, `URL`, `Headers`, `AbortController`, Web Crypto, conditional HTTP requests, and request-context timers. The repository tests the generic Web API shape in Node.js but does not claim a deployed Cloudflare or WASI runtime. | Keep the transport contract and error mapping in [Worker Fetch object-store adapter](worker-object-store.md); keep host deployment and provider behavior outside the generic txBASE contract. |
