...
| 分散レプリケーションと権威 | [分散化の進化](distributed-evolution.md) | 現在はジャーナル化された`TXRP`サイドカー永続化を含むプロセス内のバージョン付きエントリと再生の境界を実装し、ネットワーク転送、クォーラム、コンセンサス、スナップショットインストール、フォロワー読み取りは将来 |
...
| Raft論文とRaftプロジェクト資料 | 論文とプロジェクトの一次資料です。 | リポジトリ所有の`ReplicationEntry`/`ReplicationLog`スライスは、現在は固定termのプロセス内単一writerを選択し、その`TXRP`位置をカタログ更新とともにジャーナル化します。Raftはネットワーク権威の候補にとどまり、クォーラムと分散実行はありません。 | ローカルのエントリ、再生、`TXRP`契約は[分散化の進化](distributed-evolution.md)に置き、Raftは現在の実装依存ではなく将来候補として扱います。 |
