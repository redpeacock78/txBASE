...
The current index slice is intentionally local: compatible compound directions, equality-prefix candidate choice, and deterministic row-equivalent cost choice based on candidate record reads, filter evaluations, index traversal, and sort work are implemented, while a full I/O-aware model and cross-table index definitions or index-aware planning remain future work.
