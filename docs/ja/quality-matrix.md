...
| XBF-005 | 表現可能な XBF テーブルが削除状態を保ったメモリ内 DBF テーブルへ出力され、直接出力は未サポート型と制約を拒否し、スキーマ対応出力は表現可能な制約メタデータを返して保存できる。 | `src/xbf/export/mod.rs`; `src/xbf/export/convert.rs`; `src/xbf/export_tests.rs` | `exports_a_representable_xbf_table_to_dbf`; `exports_xbf_constraints_as_schema_metadata`; `saves_xbf_dbf_and_schema_sidecar`; `rejects_nonrepresentable_xbf_dbf_export_types` | Boundary |
