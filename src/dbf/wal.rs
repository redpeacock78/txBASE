use super::{DbfError, DbfTable, MemoFormat, MemoSnapshot, find_memo_path};
use std::fs;
use std::path::Path;
...
impl ByteDelta {
    pub(super) fn new(base: &[u8], target: &[u8]) -> Result<Self, DbfError> {
...
pub(super) fn decode_wal_payload(
    ...
    if payload.starts_with(DELTA_MAGIC) {
        decode_delta_payload(path, payload)
    } else {
        decode_snapshot(payload)
    }
}

fn decode_delta_payload(path: &Path, payload: &[u8]) -> Result<Option<RecoverySnapshot>, DbfError> {
